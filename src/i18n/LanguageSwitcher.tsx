...
export default function LanguageSwitcher({ className = '', variant = 'pill' }: Props) {
  ...
  const triggerClasses =
    variant === 'pill'
      ? 'bg-[#0F172A] backdrop-blur-sm flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-heading tracking-[0.18em] uppercase border border-snow/40 text-snow hover:border-amber hover:text-amber hover:bg-white/5 transition-colors duration-200'
      : 'flex items-center gap-1.5 px-2 py-1 text-xs font-semibold uppercase tracking-wider text-snow/85 hover:text-amber transition-colors duration-200';
