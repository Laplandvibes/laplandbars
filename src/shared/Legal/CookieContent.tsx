...
const COPY: Record<Lang, CookieCopy> = {
  ...
  ko: {
    ...
    analyticsBody: 'Google Analytics 4 쿠키는 방문자가 사이트를 어떻게 이용하는지(어떤 페이지가 인기 있는지, 체류 시간, 국가 단위의 접속 출처)를 파악하는 데 도움이 됩니다. 개인정보는 수집되지 않습니다.',
  ...
  fr: {
    ...
    lsPopupDesc: 'un horodatage du moment où vous avez fermé pour la dernière fois ou souscrit avec succès au popup de l\'newsletter, afin de ne pas le réafficher pendant 7 jours (ou jamais si vous vous êtes inscrit)',
...
export default function CookieContent({
  ...
  const purposeStrings: Record<Lang, { consent: string; popup: string; gaUser: string; gaSession: string; cj: string; gyg: string }> = {
    ...
    fr: {
      ...
      popup: 'Mémorise si vous avez fermé ou souscrit au popup de l\'newsletter, afin de ne pas le réafficher',
